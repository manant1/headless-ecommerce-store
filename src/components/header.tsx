...
const Header: React.FC<HeaderProps> = (props) => {
  ...
  return (
    <header className={`bg-white shadow-md mb-6`} style={{ height: 75 }}>
      <div className="section h-full w-full grid grid-cols-2">
        <div className="relative h-full w-full">
          <Link to="/products"> <StaticImage src="../images/logo.jpg" quality={95} imgStyle={{ objectFit: "contain" }}
                                             formats={["AUTO", "WEBP"]} alt="Website logo"
                                             className="absolute top-1/2 transform -translate-y-1/2"/> </Link>
        </div>
        {/* Desktop Menu */}
        <div className="md:grid md:grid-cols-3 md:h-full hidden">
          ...
          <button className={linkStyle + ` ${isActive("/cart") ? "bg-gray-100" : ""}`}><HiShoppingCart
            className={"inline"}/> Cart ({props.qty})
          </button>
        </Link>
        </div>
        {/*  Mobile Menu */}
        <div className={`w-full h-full left-0 bg-white fixed md:hidden transition-transform duration-200 ease-in-out`}
        ...
        <div className="relative md:hidden" style={{ height: 75, zIndex: 9999 }}>
          <button
